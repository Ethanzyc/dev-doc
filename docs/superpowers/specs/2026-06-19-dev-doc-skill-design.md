...
> 状态：待评审
>
> **⚠️ 演进说明（2026-06-22）**：本文件记录 v0.1 初始设计。其中"规范域动态发现"机制已演进为**规范源声明制**——Phase 1 先探测项目已有约定源（`.claude/rules/` 等），命中则复用（不强制产 `dev-doc/conventions/`）、未命中才从零累积；plan.md 声明规范源、Phase 2 checkpoint 作为独立项确认。1.2 目录结构、2.4 节、2.5 加载总表的相关描述以 **SKILL.md Phase 1.3** 为准。其余架构层设计（6 Phase / 重量分级 / 6 层 harness）仍有效。
