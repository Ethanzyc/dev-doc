...
**低保真原型（仅交互类需求，征询用户后做）**：低质量需求若同时满足——① 涉及用户交互界面；② 页面结构/交互级模糊（注意分层：流程模糊用业务流程图，字段模糊用字段清单表格，**只有页面结构/交互模糊才需要原型**）——在摆业务流程图时加一条待确认项征询用户："是否生成低保真原型辅助对齐？"（命中条件时推荐做，但由用户拍板）。**用户主动要求时不做条件判定，直接做**。做的话遵守 4 条纪律：
